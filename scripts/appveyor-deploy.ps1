...
function deploy_variant($VARIANT)
{
    ...
    if($BINTRAY_APIKEY)
    {
        ...
        Invoke-RestMethod -InFile "$ARCHIVE_NAME" -Method Put -Credential $credential -Uri "https://api.bintray.com/content/ronsaldo/phanide/lib/${VERSION_NAME}/${ARCHIVE_NAME}?publish=1"
    }
}
